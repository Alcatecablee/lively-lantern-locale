...
export async function transformAST(code: string): Promise<string> {
  ...
  try {
    ...
    console.warn('AST transform failed for entities, using string replacement');
    // Fallback to direct string replacement for HTML entities
    return fixHTMLEntitiesString(code);
  }
}
