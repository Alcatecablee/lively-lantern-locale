    ...
    <div className="min-h-screen flex flex-col bg-[#191a1f] font-sans">
      <main className="flex-1 flex flex-col items-center w-full">
        ...
        <section className="w-full flex flex-col items-center justify-center pt-7 xs:pt-8 pb-2 px-3 sm:pt-12 bg-[#191a1f] relative">
          {/* PH Badge */}
          <a
            href="https://www.producthunt.com/posts/neurolint"
            target="_blank"
            rel="noopener nofollow noreferrer"
            className="absolute top-3 left-1/2 -translate-x-1/2 sm:static sm:translate-x-0 flex items-center gap-2 bg-[#ff624f] px-4 py-2 rounded-xl font-bold text-white shadow-md text-sm hover:scale-105 active:scale-97 focus:ring-2 focus:ring-offset-1 focus:ring-[#ff624f]/60 transition-all touch-manipulation z-20"
          >
            <img
          ...
          <img
            ...
            alt="NeuroLint logo"
            className="w-28 xs:w-32 h-auto mb-3 mt-8 sm:w-40 select-none pointer-events-none"
            draggable={false}
            loading="eager"
          />
          ...
          {/* Try on PH Button */}
          <Button
            className="w-full max-w-xs py-4 text-lg rounded-xl bg-[#ff624f] hover:bg-[#fa745e] mb-4 shadow-cursor-glass active:scale-98 transition-all touch-manipulation text-white border border-[#b64334]"
            onClick={() => navigate("/app")}
            size="lg"
            aria-label="Try NeuroLint Free from Product Hunt"
          >
          ...
          <div className="w-full max-w-xs flex flex-col gap-2 items-center">
            <button
              ...
              className="text-sm text-blue-300 underline underline-offset-2 rounded px-2 py-2 focus:outline-none hover:text-blue-200 transition-all"
              aria-label={showDemo ? "Hide demo" : "Show demo"}
            >
              {showDemo ? "Hide quick demo" : "See quick demo"}
            </button>
            {showDemo && (
              <div className="w-full aspect-video bg-[#23233b] rounded-lg border border-[#262633] overflow-hidden shadow-lg animate-fade-in pointer-events-auto">
                {/* Placeholder Video/Screenshot. Replace src below with actual video if available. */}
                <video
                  src="https://user-images.githubusercontent.com/1935696/155831920-caefd1ab-6c47-4ae6-8919-2d2532fa9927.mp4"
                  className="w-full h-full object-cover"
                  controls
                  poster="/lovable-uploads/9491cce3-b317-4586-bcb1-fc0df07a440d.png"
                  aria-label="Demo video of NeuroLint in action"
                >
